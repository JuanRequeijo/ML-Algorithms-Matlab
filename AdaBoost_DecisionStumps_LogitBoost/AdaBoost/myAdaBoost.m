function [train_error, test_error] = myAdaBoost(file, fold, stump)
...
ten_fold_train_error = zeros(10,1);
    ...
    for n=1:length(tenFoldCell)
        ...
        for T=1:stump
            ...
            %choosing the alpa
            alpha_t = 0.5 * log(((1 - error_rate)/error_rate));
            ...
            for point = 1:size(currentCellTrain,1)
                ...
                y_i = curr_Y(point,:);
                W_t(point) = (W_t(point) .* exp(-((alpha_t)* y_i * label_of_stump)));
            end

            test_error = error_rate;
            stump_error(T,1) = T;
            stump_error(T,2) = alpha_t;
        ...
        for i = 1:size(currentCellTrain,1)
            ...
            for k=1:length(stump_storage)
                ...
                sign(i,1) = stump_error(k,2) * label_of_stump;
            end
